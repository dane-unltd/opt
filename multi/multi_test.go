...
func TestQuadratic(t *testing.T) {
	...
	status := solver.Solve(m1)

	t.Log(m1.ObjX, m1.Iter, status)
	...
	status = solver2.Solve(m3)

	t.Log(m3.ObjX, m3.Iter, status)
	...
	status = solver3.Solve(m4)

	t.Log(m4.ObjX, m4.Iter, status)
